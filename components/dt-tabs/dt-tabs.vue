<template>
	<view class="_tab-box" :style="{fontSize: defaultConfig.fontSize + 'rpx', color: defaultConfig.color}">
		<scroll-view id="_scroll" scroll-x class="scroll-view-h" scroll-with-animation :scroll-left="slider.scrollLeft">
...
<script>
	export default {
		...
		watch: {
			...
			activeIndex(value) {
				if(this.tagIndex !== this.activeIndex){
					this.tagIndex = this.activeIndex;
					//console.log("监测："+this.activeIndex)
				}
			},
			config(value) {
				this.updateConfig();
